...
class CartSession
{
            ...
            if ($item['product_type'] === 'equipment') {
                ...
                if ($equipment) {
                    ...
                    $validatedCart[] = [
                        ...
                        'rental_end'      => Carbon::parse($validatedRentalStart)->addHours($duration)->toDateString(),
    ...
    static public function addFoodItemToCart($food_id, $size_id, $quantity = 1)
    {
        ...
        // Check if this food+size combo already exists
        ...
        if ($existing_item !== null) {
            ...
            $cart_items[$existing_item]['total_amount'] = $quantity * $price->price;
        } else {
        ...
        foreach ($cart_items as $key => $item) {
            if ($item['product_type'] === 'equipment' && $item['equipment_id'] == $equipment_id) {
                ...
                $cart_items[$key]['total_amount'] =
                    $cart_items[$key]['quantity'] *
                    ($cart_items[$key]['rental_duration'] / 24) *
                    $cart_items[$key]['unit_price'];
                break;
            }
        }
    ...
    static public function updateCartSize($index, $newSizeId, $priceData)
    {
        ...
        if (isset($cart_items[$index])) {
            ...
            $cart_items[$index]['total_amount'] =
                $priceData->price * $cart_items[$index]['quantity'];
        }
