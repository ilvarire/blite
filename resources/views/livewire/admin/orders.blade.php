                            ...
                            <td class="px-4 py-3 text-sm">
                                <span
                                    ...
                                    @elseif ($order->status === 'processing') px-2 py-1 font-semibold leading-tight text-orange-700 bg-orange-100 rounded-full dark:text-white dark:bg-orange-600
                                         @elseif ($order->status === 'completed') px-2 py-1 font-semibold leading-tight text-green-700 bg-green-100 rounded-full dark:bg-green-700 dark:text-green-100
                ...
                @if ($selectedOrder)
                    ...
                    $selectedOrder->coupon->discount_percentage . '%off)' : 'N/A' }}
                                ...
                                @foreach ($selectedOrder->items as $item)
                                    <!-- Modal description -->
                                    <a
                                        href="{{ $item->product_type === 'food' ? route('admin.food', ['food_id' => $item->product_id]) : route('admin.equipments', ['equipment_id' => $item->product_id])}}">
                                        <div class="flex px-4 py-3 mb-8 mt-2 bg-white rounded-lg shadow-md dark:bg-gray-800">
                                            ...
                                            @if ($item->product_type === 'equipment')
                                                <p class="text-xs text-gray-600 dark:text-gray-400">
                                                    {{ $item->product->name }} <br>
                                                    Rental Starts: {{ $item->rental_start}} <br>
                                                    Duration: {{$item->rental_duration . 'hrs'}} <br>
                                                    Rental Ends: {{$item->rental_end}} <br>
                                                    Caution Fee: {{ Number::currency($item->caution_fee, 'GBP') }}<br>
                                                    Quantity: {{$item->quantity}} <br>
                                                    Total Price: {{ Number::currency($item->total, 'GBP') }}
                                                </p>
                                            @endif
                                        </div>
                                    </a>
                                    ...
                                    <div class="flex px-4 py-3 mb-8 mt-2 bg-white rounded-lg shadow-md dark:bg-gray-800">
                                        <p class="text-xs text-gray-600 dark:text-gray-400">
                                            <span class="text-xl text-gray-600 dark:text-gray-400">
                                                {{ $selectedOrder->shippingAddress->shippingFee->county->name }}
                                            </span><br>
