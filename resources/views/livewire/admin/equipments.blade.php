<div>
    ...
    <div class="w-full overflow-hidden rounded-lg shadow-xs">
        @if (session()->has('success'))
            <a class="flex items-center justify-between p-4 mb-8 text-sm font-semibold text-purple-100 bg-purple-600 rounded-lg shadow-md focus:outline-none focus:shadow-outline-purple"
                href="{{route('admin.equipment.add')}}">
                ...
                <span>Add Equipment &RightArrow;</span>
            </a>
        @endif
        <!-- Search input -->
        <div class="flex justify-between my-6 flex-1 lg:mr-32">
            ...
            <div class="relative w-xs max-w-xl mr-6 focus-within:text-purple-500">
                ...
                <input wire:model.live.debounce.500ms="search"
                    ...
                    type="text" placeholder="Search equipment" aria-label="Search" />
            </div>
            ...
            <table class="w-full whitespace-no-wrap">
                <thead>
                    <tr
                        ...
                        <th class="px-4 py-3">Caution Fee</th>
                    ...
                    @forelse ($equipments as $equipment)
                        <tr class="text-gray-700 dark:text-gray-400">
                                    ...
                                    <div class="relative hidden w-8 h-8 mr-3 rounded-full md:block">
                                        @if ($equipment->images->first())
                                            <img class="object-cover w-full h-full rounded-lg"
                                                src="{{ asset('storage/' . $equipment->images->first()->image_url) }}" alt=""
                                                loading="lazy" />
                                        @else
                                            <img class="object-cover w-full h-full rounded-lg"
                            ...
                            <td class="px-4 py-3 text-sm">
                                ...
                                {{ mb_strimwidth($equipment->description, 0, 30, '...') }}
                            </td>
                            <td class="px-4 py-3 text-sm">
                            ...
                            <td class="px-4 py-3 text-sm">
                                ...
                                {{ Number::currency($equipment->caution_fee, 'GBP') }}
                            </td>
                            <td class="px-4 py-3 text-sm">
                                <a href="javascript:;" wire:click="toggleStatus({{ $equipment->id }})">
                                    <span
                                        class="px-2 py-1 font-semibold leading-tight {{$equipment->is_featured ? 'text-green-700 bg-green-100' : 'text-red-600 bg-gray-50'}} rounded-full {{$equipment->is_featured ? 'dark:bg-green-700 dark:text-green-100' : 'text-red-600 bg-gray-50'}}">
                                        {{$equipment->is_featured ? 'featured' : 'unfeatured'}}
                                    </span>
                                </a>
                            </td>
                            <td class="px-4 py-3">
                                <div class="flex items-center space-x-4 text-sm">
                                    <flux:modal.trigger name="edit-equipment">
                                        <button
                                            wire:click="$dispatch('edit-equipment', { mode: 'edit', equipment: '{{ $equipment->slug }}'})"
                                    ...
                                    <flux:modal.trigger name="delete-equipment">
                                        <button wire:click="$dispatch('delete-equipment', { id: {{ $equipment->id }} })"
                                            class="flex items-center justify-between px-2 py-2 text-sm font-medium leading-5 text-purple-600 rounded-lg dark:text-gray-400 focus:outline-none focus:shadow-outline-gray"
                                            aria-label="Delete">
                                            <svg class="w-5 h-5" aria-hidden="true" fill="currentColor" viewBox="0 0 20 20">
                                                <path fill-rule="evenodd"
        ...
        <div
            ...
            <span class="flex items-center col-span-3">
                @if ($equipments->total() > 0)
                    Showing {{ $equipments->firstItem() }}-{{ $equipments->LastItem() }} of {{ $equipments->total() }}
                @else
    ...
    <flux:modal name="edit-equipment" class="w-full bg-white rounded-t-lg dark:bg-gray-800  sm:rounded-lg sm:max-w-xl">
        <div>
            <div class="text-center mt-8 mb-2">
                <flux:text class="text-gray-700 dark:text-gray-400">Edit equipment details
                </flux:text>
            </div>

            <div class="mb-8 bg-white rounded-lg shadow-md dark:bg-gray-800">
                <form method="POST" id="update_equipment" wire:submit.prevent="updateEquipment">
                    <div class="grid gap-6 mb-6 mt-4 md:grid-cols-2 xl:grid-cols-4">
                        @foreach ($existingImages as $image)
                            <img src="{{ asset('storage/' . $image->image_url) }}" alt="img" class="rounded-lg">
                        @endforeach
            ...
            <div class="flex">
                ...
                <button type="submit" form="update_equipment"
    ...
    <flux:modal name="delete-equipment" class="bg-white rounded-t-lg dark:bg-gray-800">
        <div>
            <div class="text-center mt-8 mb-2">
                <flux:text class="text-gray-700 dark:text-gray-400">Delete equipment?
                </flux:text>
            </div>

            <div class="mb-8 bg-white rounded-lg shadow-md dark:bg-gray-800">
                <span class="text-sm text-gray-600 dark:text-gray-400">You're about to delete this equipment</span>
                <p class="text-sm text-gray-600 dark:text-gray-400">
                    Are you sure you want to delete this equipment?</p>
            </div>
            ...
            <div class="flex">
                ...
                <button wire:click="deleteEquipment"
