<div>
    ...
    <div class="w-full overflow-hidden rounded-lg shadow-xs">
        @if (session()->has('success'))
            <a class="flex items-center justify-between p-4 mb-8 text-sm font-semibold text-purple-100 bg-purple-600 rounded-lg shadow-md focus:outline-none focus:shadow-outline-purple"
                ...
                <span>Add Event &RightArrow;</span>
            </a>
        @endif
        <!-- Search input -->
        <div class="flex justify-between my-6 flex-1 lg:mr-32">
            ...
            <div class="relative w-xs max-w-xl mr-6 focus-within:text-purple-500">
                ...
                <input wire:model.live.debounce.500ms="search"
                    ...
                    type="text" placeholder="Search gallery" aria-label="Search" />
            </div>
                    ...
                    @forelse ($galleries as $gallery)
                        <tr class="text-gray-700 dark:text-gray-400">
                                    ...
                                    <div class="relative hidden w-8 h-8 mr-3 rounded-full md:block">
                                        @if ($gallery->images->first())
                                            <img class="object-cover w-full h-full rounded-lg"
                                                src="{{ asset('storage/' . $gallery->images->first()->image_url) }}" alt=""
                                                loading="lazy" />
                                        @else
                                            <img class="object-cover w-full h-full rounded-lg"
                            ...
                            <td class="px-4 py-3 text-sm">
                                ...
                                {{ mb_strimwidth($gallery->description, 0, 30, '...') }}
                            </td>
                            <td class="px-4 py-3 text-sm">
                                <a href="javascript:;" wire:click="toggleStatus({{ $gallery->id }})">
                                    <span
                                        class="px-2 py-1 font-semibold leading-tight {{$gallery->is_featured ? 'text-green-700 bg-green-100' : 'text-red-600 bg-gray-50'}} rounded-full {{$gallery->is_featured ? 'dark:bg-green-700 dark:text-green-100' : 'text-red-600 bg-gray-50'}}">
                                        {{$gallery->is_featured ? 'featured' : 'unfeatured'}}
                                    </span>
                                </a>
                            </td>
                            <td class="px-4 py-3">
                                <div class="flex items-center space-x-4 text-sm">
                                    <flux:modal.trigger name="edit-gallery">
                                        <button
                                            wire:click="$dispatch('edit-gallery', { mode: 'edit', gallery: '{{ $gallery->slug }}'})"
                                    ...
                                    <flux:modal.trigger name="delete-gallery">
                                        <button wire:click="$dispatch('delete-gallery', { id: {{ $gallery->id }} })"
                                            class="flex items-center justify-between px-2 py-2 text-sm font-medium leading-5 text-purple-600 rounded-lg dark:text-gray-400 focus:outline-none focus:shadow-outline-gray"
                                            aria-label="Delete">
                                            <svg class="w-5 h-5" aria-hidden="true" fill="currentColor" viewBox="0 0 20 20">
                                                <path fill-rule="evenodd"
    ...
    <flux:modal name="edit-gallery" class="w-full bg-white rounded-t-lg dark:bg-gray-800  sm:rounded-lg sm:max-w-xl">
        <div>
            <div class="text-center mt-8 mb-2">
                <flux:text class="text-gray-700 dark:text-gray-400">Edit event
                </flux:text>
            </div>

            <div class="mb-8 bg-white rounded-lg shadow-md dark:bg-gray-800">
                <form method="POST" id="update_gallery" wire:submit.prevent="updateGallery">
            ...
            <div class="flex">
                ...
                <button type="submit" form="update_gallery"
    ...
    <flux:modal name="delete-gallery" class="bg-white rounded-t-lg dark:bg-gray-800">
        <div>
            <div class="text-center mt-8 mb-2">
                <flux:text class="text-gray-700 dark:text-gray-400">Delete event?
                </flux:text>
            </div>

            <div class="mb-8 bg-white rounded-lg shadow-md dark:bg-gray-800">
                <span class="text-sm text-gray-600 dark:text-gray-400">You're about to delete this event</span>
