        ...
        <form wire:submit.prevent="storeFood" enctype="multipart/form-data" id="add_food">
            ...
            <div class="flex gap-6">
                @forelse($sizes as $size)
                    <label class="block text-sm w-16">
                        <span class="text-gray-700 font-bold dark:text-gray-400">
                            {{ $size->label }}
                        </span>
                        <input type="text" wire:model="prices.{{ $size->id }}.price" value="{{ old('price') }}" name="money"
                            class="block w-full mt-1 text-sm dark:text-gray-300 dark:border-gray-600 dark:bg-gray-700 focus:border-purple-400 focus:outline-none focus:shadow-outline-purple form-input"
                            placeholder="00.00" />
                        <span class="text-gray-700 font-bold dark:text-gray-400">
                            Av. Weight(kg - {{ $size->label }})
                        </span>
                        <input type="text" wire:model="prices.{{ $size->id }}.weight" value="{{ old('weight') }}"
                            name="money"
                            class="block w-full mt-1 text-sm dark:text-gray-300 dark:border-gray-600 dark:bg-gray-700 focus:border-purple-400 focus:outline-none focus:shadow-outline-purple form-input"
                            placeholder="00.00" />
            ...
            <div class="mb-4">
                @error('prices.*')
                    <br> <span class="text-xs text-red-600 dark:text-red-400 mb4">
                        {{$message}}
                    </span>
                @enderror
                @error('prices.*.price')
                ...
                @enderror
                @error('prices.*.weight')
                    <br><span class="text-xs text-red-600 dark:text-red-400 mb-4">
                        {{$message}}
                    </span>
                @enderror
            </div>
